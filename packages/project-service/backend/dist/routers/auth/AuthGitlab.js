...
router.post('/get_access_token', async (req, res) => {
    ...
    try {
        ...
        const redirect_uri = process.env.GITLAB_REDIRECT_URI ?? 'http://localhost:5173/import-repos';
        ...
        const tokenRes = await axios.post('https://gitlab.com/oauth/token', params.toString(), {
            headers: {
...
router.get('/get_repositories', async (req, res) => {
    ...
    try {
        const response = await axios.get('https://gitlab.com/api/v4/projects?membership=true&per_page=100', {
            headers: {
        ...
        const insertSql = `
                        ...
                        `;
        void Promise.all(Repos.map((repo) => pool
...
//# sourceMappingURL=AuthGitlab.js.map